...
|---|---|---|---|
| 1.1 | Temporal statutory minimums | **Adopt** | Correct and important, and it paid off. Rules are looked up by the tender's `noticePublicationDate` against entries carrying `effective_from` / `effective_to`. The bid-period minimum really does change inside the covered window: пункт 34 set seven days for every subject from 2022-10-19, and Постанова КМУ № 382 від 02.04.2024 split it into seven days for товари/послуги and 14 for роботи with effect from **09.04.2024**. All figures were read from the official text, including the superseded edition for the earlier one. The table still covers only the periods the dataset spans; it is not a complete legal history. |
